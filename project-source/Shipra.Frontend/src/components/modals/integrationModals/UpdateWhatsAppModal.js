...
const UpdateWhatsAppModal = (props) => {
  const { open, onClose, getAllWhatsappActivate, UpdateData } = props;
  ...
  useEffect(() => {
    if (UpdateData) {
      if (allWhatsappService && allWhatsappService.length > 0) {
        let defRecord = allWhatsappService.find(
          (x) => x?.whatsAppLookupId == UpdateData?.whatsAppLookupId
        );
        ...
        setWhatsAppConfigData(updatedConfig);
        setIsDefault(UpdateData?.isDefault);
      }
    }
  }, [allWhatsappService]);
  ...
  useEffect(() => {
    if (whatsAppConfigData) {
      const parsedData = JSON.parse(whatsAppConfigData);
