...
function Products1(props) {
  ...
  const handleSelectAllClick = (event) => {
    ...
    setIsAllVariantSelected(!isAllVariantSelected);
  };
  useEffect(() => {
  ...
  function updateSKUCheckedByUuid(array, uuid, isSKUChecked) {
    const updatedProducts = array?.map((product) => {
      if (product.uuid === uuid) {
        return {
          ...product,
          SKUChecked: !product?.SKUChecked,
        };
      }
      return product;
    });
    console.log(updatedProducts, isSKUChecked);
  ...
  const handleCombinationIsSkuCheckd = (event, name, item) => {
    ...
    let updatedTempCombinations = updateSKUCheckedByUuid(
      tempCombinations,
      item?.uuid,
      item?.SKUChecked
    );
    let updatedTempCombinationsWithStations = updateSKUCheckedByUuid(
      tempCombinationsWithAllStation,
      item?.uuid,
      item?.SKUChecked
    );

    setCombinations(updatedCombinations);
    setTempCombinations(updatedTempCombinations);
    setTempCombinationsWithAllStation(updatedTempCombinationsWithStations);
  };

  function updateSKUCheckedByUuidForAll(array) {
    const updatedProducts = array?.map((product) => {
      return {
        ...product,
        SKUChecked: isAllVariantSelected,
      };
    });

    return updatedProducts;
  }
  const handleCombinationIsSkuCheckdForAll = () => {
    let updatedCombinations = updateSKUCheckedByUuidForAll(combinations);
    let updatedTempCombinations =
      updateSKUCheckedByUuidForAll(tempCombinations);
    let updatedTempCombinationsWithStations = updateSKUCheckedByUuidForAll(
      tempCombinationsWithAllStation
    );
  ...
  const handleSetDefaultSkusWithStations = (filteredArray) => {
    ...
    setTempCombinationsWithAllStation(structureArray);
  };
  useEffect(() => {
    console.log(
      "tempCombinationsWithAllStation",
  ...
  const handleStationQuantity = (e, item) => {
    ...
    //#region
    const actualArr = updateQuantityByUuid(
      tempCombinationsWithAllStation,
      item?.uuid,
      qty
    );
    // debugger;
    let updatedArr = updateIsAddedQtyAgainstSku(actualArr, item);
    setTempCombinationsWithAllStation(updatedArr);
    //#endregion
    //#region
  ...
  const handleStationLowQuantity = (e, item) => {
    let qty = eval(e.target.value) ? eval(e.target.value) : 0;
    //#region
    const actualArr = updateLowQuantityByUuid(
      tempCombinationsWithAllStation,
      item?.uuid,
      qty
    );
    let updatedArr = updateIsAddedQtyAgainstSku(actualArr);
    setTempCombinationsWithAllStation(item);
    //#endregion
    //#region
  ...
  const handleAddQty = () => {
    ...
    let data = tempCombinationsWithAllStation.filter(
      (x) => x.isAddedStationQty == true
    );
    setTempCombinations(data);
  };
  useEffect(() => {
    let targetedData = tempCombinationsWithAllStation.filter(
      (x) =>
                ...
                <Box sx={{ width: "100%" }}>
                  <Paper sx={{ width: "100%", mb: 2 }}>
                    ...
                    <EnhancedTableToolbar
                      numSelected={
                        combinations?.filter((x) => x.SKUChecked == true).length
                      }
                          ...
                          <TableRow>
                            <TableCell padding="checkbox">
                              <Checkbox
                                ...
                                indeterminate={
                                  selected.length > 0 && isAllVariantSelected
                                }
                                checked={
                        ...
                        {optionsCheckbox ? (
                          <TableBody>
                            {combinations.map((combination, index) => {
                              const isItemSelected = combination.SKUChecked; //isSelected(index);
                              const labelId = `enhanced-table-checkbox-${index}`;
                              return (
                                <TableRow
                                  hover
                                  onClick={(event) =>
                                    handleClick(event, index, combination)
                                  }
                                  role="checkbox"
          ...
          <DialogContentText id="alert-dialog-slide-description">
            ...
            <Grid container spacing={2} sx={{ mt: "5px" }}>
              ...
              <Grid item xs={12}>
                <Table fullWidth aria-label="customized table">
                  <TableBody>
                    <StyledTableRow width={"100%"}>
                      <StyledTableCell>
                        <SelectComponent
                          name="reason"
                          options={productStationsWithPleaseSelectOp}
