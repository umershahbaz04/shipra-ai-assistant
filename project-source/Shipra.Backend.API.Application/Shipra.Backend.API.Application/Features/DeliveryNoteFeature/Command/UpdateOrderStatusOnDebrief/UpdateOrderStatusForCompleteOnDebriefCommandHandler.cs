...
public class UpdateOrderStatusForCompleteOnDebriefCommandHandler : RequestHandlerBase<UpdateOrderStatusForCompleteOnDebriefCommand, ServiceResultDTO>
{
  ...
  public UpdateOrderStatusForCompleteOnDebriefCommandHandler(IMediator mediator, IEmployeeRepository employeeRepository, IOrderRepository orderRepository, IDriverRepository driverRepository, IDeliveryTaskRepository deliveryTaskRepository, IDeliveryNoteRepository deliveryNoteRepository, IOrderTrackingHistoryRepository historyRepository, IServiceProvider serviceProvider, ILogger<UpdateOrderStatusForCompleteOnDebriefCommandHandler> logger) : base(serviceProvider, logger)
  {
    _mediator = mediator;
  ...
  protected override async Task<ServiceResultDTO> HandleRequest(UpdateOrderStatusForCompleteOnDebriefCommand request, CancellationToken cancellationToken)
  {
      ...
      if (oDeliveryNoteDetail is not null)
      {
        ...
        if (oOrder is not null)
        {
          ...
          if (oDeliveryTask is not null)
          {
            ...
            var oOrderTrackingHistory = await _historyRepository.CreateOrderTrackingHistory(OrderTrackingHistory.CreateOrderTrackingHistory(oOrder!.OrderId!, (int)EnumCarrierTrackingStatus.Delivered, "Order Status updated by Shipra App For Complete On Debrief", _currentUser.EmployeeId!, createdByName, (int)EnumOrderHistoryType.ShipraApp));
