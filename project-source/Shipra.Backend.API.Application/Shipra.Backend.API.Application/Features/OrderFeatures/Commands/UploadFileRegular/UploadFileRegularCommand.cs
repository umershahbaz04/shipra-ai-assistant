...
namespace Shipra.Backend.API.Application.Features.OrderFeatures.Commands.UploadFileRegular;
public class UploadFileRegularCommand : IRequest<ServiceResultDTO>
{
  public int CountryId { get; set; }
  public IFormFile? File { get; set; }
}
public class UploadFileRegularCommandHandler : RequestHandlerBase<UploadFileRegularCommand, ServiceResultDTO>
{
  ...
  public UploadFileRegularCommandHandler(IStoreRepository storeRepository, IEmployeeRepository employeeRepository, IOrderBoxRepository orderBoxRepository, IProductStationRepository productStationRepository, IClientRepository clientRepository, IPaymentMethodLookupRepository paymentMethodLookupRepository, IS3Service s3Service, ICountryRepository countryRepository, IOrderRepository orderRepository, IWebHostEnvironment webHostEnvironment, IServiceProvider serviceProvider, ILogger<UploadFileRegularCommandHandler> logger) : base(serviceProvider, logger)
  {
  ...
  protected override async Task<ServiceResultDTO> HandleRequest(UploadFileRegularCommand request, CancellationToken cancellationToken)
  {
    ...
    try
    {
      ...
      if (request.File?.ContentType != null)
      {
        ...
        if (dataList != null && dataList.Count > 0)
        {
          ...
          UDTOrderDetailResponse response = UDTOrderDetailSimplified.ConvertoRegularOrderDetail(dataList, request.CountryId, countries, cities, provinces, states, areas, pinCodes, paymentMethods, allProductStations, allOrderBox, client.DefaultProductStationId.GetValueOrDefault(), _mapper, allStores!, allEmployees,allSalesPerson,client);
          ...
          for (int rowIndex = 0; rowIndex < response.UDTOrderDetail!.Count; rowIndex++)
          {
            ...
            if (item.StoreId.GetValueOrDefault() == 0)
            {
              item.StoreId = client!.DefaultStoreId;
            }
            if (item.StationId == 0)
            {
              item.StationId = client!.DefaultProductStationId.GetValueOrDefault();
            }
            item.OrderTypeId = (int)EnumOrderType.Regular;
  ...
  {
    int noOfPieces;
    int.TryParse(rowData[columnNames.IndexFor("NoOfPieces")], out noOfPieces);
    var product = new UDTOrderDetailSimplified()
    {
      ...
      StoreCode = rowData[columnNames.IndexFor("StoreCode")],
