...
public sealed class ExportToExcelProductInventory
{
  public byte[] ExportToExcel(dynamic data, string title)
  {
    ...
    if (!IsDataEmpty)
    {
      dataTable.Columns.Remove("ProductStockId"); 
      dataTable.Columns.Remove("RowNum"); 
      dataTable.Columns.Remove("TotalCount"); 
      dataTable.Columns.Remove("QuantityInComing"); 
    }
    #endregion
    string[] columnsToTake = { "StationName", "ProductName", "SKU", "Price", "QuantityInComing", "QuantitySchedule", "QuantityCommited", "QuantityDamage","LowQuantityLimit", "QuantityOnOrder", "QuantityAvailable",  "VarientOption", "TotalInTransitCount" };
    bool showSrNo = true;
