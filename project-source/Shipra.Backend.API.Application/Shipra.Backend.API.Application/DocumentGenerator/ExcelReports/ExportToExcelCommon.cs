...
public sealed class ExportToExcelCommon
{
  ...
  public byte[] ExportToExcelWithDynamicList(List<dynamic> data, string title, string[]? columnsToRemove = null)
  {
    byte[]? result = null;
    string heading = title;
    ExcelExportHelper excelExportHelper = new ExcelExportHelper();
    ...
    if (!IsDataEmpty)
    {
      ...
      {
        foreach (var columName in columnsToRemove)
        {
          if (dataTable.Columns.Contains(columName))
          {
            dataTable.Columns.Remove(columName);
