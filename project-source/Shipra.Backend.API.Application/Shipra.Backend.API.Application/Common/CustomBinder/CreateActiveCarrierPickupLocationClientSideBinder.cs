...
public class CreateActiveCarrierPickupLocationClientSideBinder : IModelBinder
{
  public async Task BindModelAsync(ModelBindingContext bindingContext)
  {
    ...
    using (var reader = new StreamReader(request.Body, Encoding.UTF8, leaveOpen: true))
    {
      ...
      try
      {
        ...
        var clientRequest = JsonConvert.DeserializeObject<PickupLocationAddressClientSideModel>(rawRequestBody,
     new JsonSerializerSettings { MissingMemberHandling = MissingMemberHandling.Ignore });

        if (clientRequest != null)
        {
          // Convert client-side request to CreateOrderCommand
          var createOrderCommand = AddressConversionHelper.PickupLocationAddressModel(clientRequest);
